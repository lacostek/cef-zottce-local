...
impl Manager {
    ...
    pub fn create_browser(&mut self, id: u32, cbs: CallbackList, url: &str) {
        ...
        let handle_url = if url.starts_with("localGame://") {
            let cef_dir = crate::utils::cef_dir();
            let url_add = cef_dir.join(&url["localGame://".len()..]);
            url_add.to_string_lossy().to_string()
        } else {
            url.to_string()
        };

        crate::browser::cef::create_browser(client.clone(), &handle_url);
